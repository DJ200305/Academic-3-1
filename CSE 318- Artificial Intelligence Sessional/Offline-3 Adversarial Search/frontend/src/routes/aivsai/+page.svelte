<script>
	...
	let turn = "AI_1 Move:"
	let first = 0
	let selection = localStorage.getItem('mode') || 'default';
	let rcnt = 0
	let bcnt = 0
	let ztime = 90
	let inttime = 0;
	...
	async function start(){
		...
		let etime = Date.now();
		let time = (etime - stime) / 1000;
		await fetch('http://127.0.0.1:8000/api/logtime',{
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
			body: JSON.stringify({aitime: time})
		})
	}
	async function txtdeletion(){
	...
	async function ai_1_move(){
		console.log(selection)
        const ai_1_move = await fetch(`http://127.0.0.1:8000/api/ai_move?mode=${selection}`,{method: 'GET'})
		const output1 = await ai_1_move.json();
		const status = output1.output[0];
		console.log(status)
		if (status === 'Game Over'){
			over = true
			winner = [output1.output[10] , '(AI)']
			return 
		}
		for(let i=0;i<9;i++){
			state[i] = output1.output[i+1].split(' ')
		}	
		await counterR()
		await counterB()
		console.log(state)
        const ai_2_move = await fetch('http://127.0.0.1:8000/api/ai2_move',{method: 'GET'})
        const output = await ai_2_move.json();
		console.log("testing")
	...
	async function write(){
		...
		console.log(line.join('\n'));
		await fetch('http://127.0.0.1:8000/api/aiwrite',{ 
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
...
{#if winner !== null}
    <Winner winner = {winner} reset = {reset} timerend = {timerend} txtdeletion={txtdeletion}/> 
{:else}
<div class = "wrapper">
  <div class = "main">	
    <div class="state">
